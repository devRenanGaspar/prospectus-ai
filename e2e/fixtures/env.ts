...
// The account's real WhatsApp number, computed once so `crmTable` and
// `agentMemoryTable` (per-tenant table names keyed by phone) don't repeat it.
const agentPhone = required("E2E_AGENT_PHONE");
...
export const env = {
  ...
  // A real personal address, not published in the repo -- see e2e/.env.example.
  appEmail: required("E2E_APP_EMAIL"),
  ...
  // --- n8n ---
  // Real infra hostnames -- not published in the repo, see e2e/.env.example.
  /** Editor + REST API host. NOT the webhook host. */
  n8nApiUrl: required("E2E_N8N_API_URL"),
  /** Webhook processor host. Serves /webhook/* only; has no /api/v1. */
  n8nWebhookUrl: required("E2E_N8N_WEBHOOK_URL"),

  // --- the fixed test lead and its account ---
  // Real WhatsApp numbers and a real personal inbox for the meeting invite --
  // not published in the repo. IDs below are fine in the open: without the
  // phone/email/webhook values above they don't let anyone act on the account.
  userId: "40090921-e613-45b8-b6ab-ac1c77383cbd",
  leadId: "cc9ab02a-16b1-4d16-89d6-c68c22b93d7b",
  leadPhone: required("E2E_LEAD_PHONE"),
  /** The account's WhatsApp number. Also the per-tenant table suffix. */
  agentPhone,
  crmTable: `crm_${agentPhone}`,
  agentMemoryTable: `n8n_${agentPhone}`,
  /** Address the agent books the meeting invite to, in TESTE 5. */
  leadEmail: required("E2E_LEAD_EMAIL"),
  ...
  // --- webhook paths (no auth on either — the path itself is the only gate,
  // so it lives in .env.e2e like a credential, not in this file) ---
  webhooks: {
    /** Sends a WhatsApp message AS the lead. Accepts { mensagem }. */
    testSender: required("E2E_WEBHOOK_TEST_SENDER_PATH"),
    /** Forces the send queue instead of waiting for the 15-minute schedule. */
    forceSendQueue: required("E2E_WEBHOOK_FORCE_SEND_QUEUE_PATH"),
  },
